...
void	print_nbr_case2(t_print *p, intmax_t n, int space)
{
	char c;
...
void	print_nbr_case4(t_print *p, intmax_t n, int space, int pads)
{
	(!(p->flag & _F_MINUS)) && put_nchar(' ', space);
	process_sp_plus_flag(p, n);
	put_nchar('0', pads);
...
void	print_nbr_driver(t_print *p, intmax_t n, int space, int pads)
{
	if (!space && !pads)
		print_nbr_case1(p, n);
	else if (space && !pads)
